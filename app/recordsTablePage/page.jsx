  ...
  return (
    <div className={styles.leaderboardContainer}>
      <table className={styles.table}>
        <thead>
          <tr>
            <th className={styles.th}>Place</th>
            <th className={styles.th}>Name</th>
            <th className={styles.th}>Surname</th>
            <th className={styles.th}>Level</th>
            <th className={styles.th}></th> {/* Пустая ячейка для SVG */}
          </tr>
        </thead>
        <tbody>
          {sortedData.map((user, index) => (
            <tr key={index}>
              ...
              <td className={styles.td}>
                {index < 3 && (
                  <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" className={getTrophyClass(index + 1)} viewBox="0 0 25 25">
